...
(function(obj,undefined){
	...
	var replacementP=[16,7,20,21,29,12,28,17,1,15,23,26,5,18,31,10,2,8,24,14,32,27,3,9,19,13,30,6,22,11,4,25];
	var replacementIP=[58,50,42,34,26,18,10,2,60,52,44,36,28,20,12,4,62,54,46,38,30,22,14,6,64,56,48,40,32,24,16,8,57,49,41,33,25,17,9,1,59,51,43,35,27,19,11,3,61,53,45,37,29,21,13,5,63,55,47,39,31,23,15,7];
	...
	function encode(arr,key){//DES加密主函数
		...
		var res=[],L=[],R=[],tem=[],temres=[],i=0,j,m;
		for(;i<arr.length;i++){
			L[i]=[[]],R[i]=[[]];
			for(m=0;m<32;m++){
			...
			tem=R[i][16].concat(L[i][16]);
			
			for(var m=0;m<64;m++){
				temres[m]=tem[replacementIP[m]-1];
			}
			res[i]=temres;
		}
		return res;
	...
	function ext(L,R,key){//F函数
		var temKey=[],temS=[],res=[],ret=[];
		for(var i=0;i<48;i++){
			temKey[i]=parseInt(R[selectE[i]-1]+key[i])%2;
			if(i%6==0||i%6==5){
				if(temS[Math.floor(i/6)]==null){
					temS[Math.floor(i/6)]=[];
					temS[Math.floor(i/6)][0]=temKey[i];
				}else{
					temS[Math.floor(i/6)][0]+=""+temKey[i];
				}
			}else{
				if(!temS[Math.floor(i/6)][1]){
					temS[Math.floor(i/6)][1]=""+temKey[i];
				}else{
					temS[Math.floor(i/6)][1]+=""+temKey[i];
				}
			}
		}
		temS.forEach(function(arr,i){
			res=res.concat(tenToBinary(S[i][strToBinary(arr[0])][strToBinary(arr[0])]));
		});
		
		for(i=0;i<32;i++){
			ret[i]=res[replacementP[i]-1];
		}
		return ret;
	...
	function tenToBinary(num){
		var res=[0,0,0,0];
		if(num>=8){res[0]=1;num-=8;}
		if(num>=4){res[1]=1;num-=4;}
		if(num>=2){res[2]=1;num-=2;}
		if(num>=1){res[3]=1;}
		return res;
	}
